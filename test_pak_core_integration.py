#!/usr/bin/env python3
"""
Integration tests for pak.py
Tests the main CLI functionality end-to-end by running pak.py as a subprocess
"""
...
class PakIntegrationTester:
    """Integration test class for pak.py CLI functionality"""
    
    def __init__(self):
        self.test_dir = None
        self.pak_path = Path(__file__).parent / "pak.py"
...
''')
    ...
    def run_pak(self, args, input_data=None):
        ...
        cmd = ["python3", str(self.pak_path)] + args
...
def test_pak_core_pack_basic(pak_tester):
    ...
    result = pak_tester.run_pak(['pack', '.', '-c', 'none', '-o', 'test.pak'])
...
def test_pak_core_pack_with_compression_levels(pak_tester):
    ...
    for level in compression_levels:
        ...
        result = pak_tester.run_pak(['pack', '.', '-c', level, '-o', output_file])
...
def test_pak_core_pack_with_extensions_filter(pak_tester):
    ...
    result = pak_tester.run_pak(['pack', '.', '--ext', '.py', '-o', 'python_only.pak'])
...
def test_pak_core_list_archive(pak_tester):
    ...
    pak_tester.run_pak(['pack', '.', '-o', 'test.pak'])
    
    # List archive contents
    result = pak_tester.run_pak(['list', 'test.pak'])
...
def test_pak_core_list_detailed(pak_tester):
    ...
    result = pak_tester.run_pak(['list-detailed', 'test.pak'])
...
def test_pak_core_verify_archive(pak_tester):
    ...
    # Verify archive
    result = pak_tester.run_pak(['verify', 'test.pak'])
...
def test_pak_core_extract_archive(pak_tester):
    ...
    # Extract archive
    result = pak_tester.run_pak(['extract', 'test.pak', '-d', str(extract_dir)])
...
def test_pak_core_extract_with_pattern(pak_tester):
    ...
    result = pak_tester.run_pak(['extract', 'test.pak', '-d', str(extract_dir), '-p', '.*\\.py$'])
...
''')
    ...
    # Extract diff
    result = pak_tester.run_pak(['extract-diff', str(original_file), str(modified_file), '-o', 'changes.diff'])
...
''')
    ...
    # Verify diff
    result = pak_tester.run_pak(['verify-diff', str(diff_file)])
...
def test_pak_core_quiet_mode(pak_tester):
    ...
    result = pak_tester.run_pak(['pack', '.', '-q', '-o', 'quiet_test.pak'])
...
if __name__ == "__main__":
    ...
    print("🧪 Running pak.py integration tests manually...")
    
    try:
        # Test basic pack
        print("📦 Testing basic pack...")
        result = tester.run_pak(['pack', '.', '-o', 'manual_test.pak'])
        ...
        # Test list
        print("📋 Testing list...")
        result = tester.run_pak(['list', 'manual_test.pak'])
